...
qint64 MainWindow::sendBytesDirect(const QByteArray &data)
{
    if (!serialPort) return -1;
    std::lock_guard<std::mutex> lk(serialWriteMutex);
    // Write directly; may be called from non-GUI thread. QSerialPort is not guaranteed thread-safe,
    // but serializing access with mutex reduces risk. If issues occur, use main-thread sending.
...
void MainWindow::on_openBt_clicked()
{
    ...
    if(serialPort->open(QIODevice::ReadWrite) == true){
       ...
       QString err = QString("串口打开失败！！！\n端口: %1\n错误: %2").arg(serialPort->portName()).arg(serialPort->errorString());
       QMessageBox::critical(this, "错误提示", err);
