...
//读取相机中的图像 (使用QImage替代OpenCV Mat，移除OpenCV依赖)
int CMvCamera::ReadBuffer(QImage &image)
{
    ...
    unsigned char* pDate = (unsigned char *)malloc(nRecvBufSize);
    ...
    {
        // 单通道灰度图
        QImage tmp(pDate, stImageInfo.nWidth, stImageInfo.nHeight, stImageInfo.nWidth, QImage::Format_Grayscale8);
        image = tmp.copy(); // 复制数据，因为后面会释放缓冲区
    }
    else
    {
        // 转换图像格式为BGR8
        ...
        // QImage不直接支持BGR格式，通过当作RGB来创建后调用rgbSwapped()修正通道顺序
        QImage tmp(m_pBufForSaveImage, stImageInfo.nWidth, stImageInfo.nHeight, stImageInfo.nWidth * 3, QImage::Format_RGB888);
