    ...
    , ui(new Ui::MainWindow)
{
    ...
    // Use the application's global icon if set; otherwise try common resource names
    QIcon appIcon = QApplication::windowIcon();
    if (appIcon.isNull()) {
        // Try several common resource names (ico, png, and the legacy .ioc if present)
        appIcon = QIcon(":/img/photo.ico");

    }
    if (!appIcon.isNull()) this->setWindowIcon(appIcon);
    else qDebug() << "MainWindow: application icon not found in resources (tried .ico .png .ioc)";
